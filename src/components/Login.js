...
const Login = () => {
  ...
  return (
    <div>
      ...
      <div className='md:h-[85vh] md:w-[70vh] top-20 md:left-[70vh] left-5 absolute bg-[#000000d3]'>
        <h1 className='text-white md:text-3xl font-bold md:mt-16 mt-2 md:mx-10 md:mb-7 mx-5 mb-5'>{isSignInPage?'Sign In':'Sign up' }</h1>
        <form className="md:ml-2 md:mx-1 ml-3 md:text-base text-sm" onSubmit={(e)=>e.preventDefault()}>
          ...
          <div className='text-zinc-500 md:mt-4 ml-10 mb-5 font-medium flex gap-1'>{!isSignInPage?'Already registered?':'New to Netflix?' }<p className='text-white font-medium cursor-pointer' onClick={handleToggle}>{!isSignInPage?'Sign In now.':'Sign up now.' }</p></div>
        </form>
      </div>
    </div>
  )
